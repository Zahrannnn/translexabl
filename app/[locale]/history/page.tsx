          ...
          <div className="flex items-center justify-center min-h-[60vh]">
            <div className="text-center">
              ...
              <p className="mt-4 text-muted-foreground">
                {authLoading ? t('loading.auth') : t('loading.transactions')}
              </p>
            </div>
          </div>
        </div>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-sm text-muted-foreground">{t('accountInfo.email')}:</span>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="text-center">
                ...
                <div className="text-sm text-muted-foreground">{t('summary.totalTransactions')}</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-green-600">
                  {transactions.filter(t => t.status === 'success').length}
                </div>
                <div className="text-sm text-muted-foreground">{t('summary.successful')}</div>
              </div>
              <div className="text-center">
                ...
                <div className="text-sm text-muted-foreground">{t('summary.totalSpent')}</div>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            {transactions.length === 0 ? (
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-semibold text-muted-foreground mb-2">{t('transactions.noTransactions')}</h3>
                <p className="text-muted-foreground mb-4">{t('transactions.noTransactionsDesc')}</p>
                <Link href="/pricing">
                  <Button className="hover-lift">
                    <CreditCard className="h-4 w-4 mr-2" />
                    {t('transactions.purchaseCredits')}
                  </Button>
                </Link>
              </div>
            ) : (
                ...
                {transactions.map((transaction) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-xs text-muted-foreground mt-1">
                          {t('transactions.id')}: {transaction.transactionId}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mt-3 pt-3 border-t border-muted grid grid-cols-1 md:grid-cols-2 gap-2 text-xs text-muted-foreground">
                      <div>{t('transactions.orderId')}: {transaction.orderId}</div>
                      <div>{t('transactions.merchantOrder')}: {transaction.merchantOrderId}</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="flex items-center justify-between mt-8">
            <div className="text-sm text-muted-foreground">
              {t('pagination.showing', {
                ...
                to: Math.min(pagination.currentPage * pagination.perPage, pagination.totalRecords),
