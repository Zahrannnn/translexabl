...
export function Navbar() {
  ...
  const translationItems = [
    ...
    },
    {
      ...
      href: "/test-translate-gemini",
      icon: <SparklesIcon className="h-4 w-4" />,
    },
  ];

  return (
    <nav className="sticky top-3 sm:top-6 z-50 modern-card border-b border-border/20 backdrop-blur-xl w-[95%] sm:w-[90%] mx-auto py-2 sm:py-3">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8">
        <div className="flex justify-between items-center h-12 sm:h-16">
          {/* Logo */}
          <Link
            href="/"
            className="flex items-center space-x-2 sm:space-x-3 hover-lift group"
          >
            <div className="relative">
              <div className="w-8 h-8 sm:w-10 sm:h-10 bg-gradient-to-br from-primary to-accent rounded-xl flex items-center justify-center shadow-lg group-hover:shadow-glow transition-all duration-300">
          ...
          {/* Mobile menu button and compact language switcher */}
          <div className="md:hidden h-fit flex items-center space-x-1 sm:space-x-2">
            {/* Compact Language Switcher for Mobile Header */}
            <LanguageSwitcher compact={true} className="hidden xs:block" />

            <Button
              variant="ghost"
              size="sm"
              onClick={() => setIsOpen(!isOpen)}
              className="modern-card p-1.5 sm:p-2 hover:bg-primary/10 h-8 w-8 sm:h-10 sm:w-10"
            >
              {isOpen ? (
        ...
        {isOpen && (
          <div className="md:hidden">
            <div className="px-2 pt-2 pb-3 space-y-1 modern-card mt-2 rounded-2xl border border-border/20 shadow-lg shadow-primary/10">
              ...
              <LanguageSwitcher showInMobileMenu={true} />
